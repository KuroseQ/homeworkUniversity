...
public class HomeWork2 {
    ...
    public static void validateLogin(String login) throws LoginValidationException {
        int countLargeLetters = 0;
        int countSmallLetters = 0;
        int countNumbers = 0;
        int countUnderscores = 0;

        for (int i = 0; i < login.length(); i++) {
            String element = String.valueOf(login.charAt(i));
        ...
        int countAllChar = countLargeLetters + countSmallLetters + countNumbers + countUnderscores;

        if (login.length() > 20 || countAllChar != login.length()
                || countLargeLetters == 0 || countSmallLetters == 0
                || countNumbers == 0 || countUnderscores == 0) {
